...
Known C-side limitation: when multiple Siemens raw-data series share a ProtocolName/SeriesDescription stem after reproin parsing (e.g. a main BOLD's PhysioLog + the matching SBRef's PMU blob — both encode `func-bold_task-rest_acq-dualecho_run-1_dicom` as the BIDS root), dcm2niix writes both to the same filename and the second overwrites the first. The rescue here recovers whatever survived in `derivatives/scanner/`; in practice that's the later-arriving series (typically the SBRef PMU rather than the BOLD's PhysioLog). Splitting them requires dcm2niix-side filename disambiguation that the C path doesn't currently do — tracked as a separate follow-up. Multi-session subjects are deferred: the rescue prints a warning and skips them because the source filename doesn't encode `_ses-` and disambiguating across sessions needs acquisition-time matching the provenance TSV doesn't surface for raw-data series.
