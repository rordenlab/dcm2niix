...
C-side `_acq-{physiolog,pmu}` disambiguation (`saveDcm2NiiCore` physio branch). To avoid the collision described above, the physio writer post-processes the `nii_createFilename`-produced `baseName` to splice in an `_acq-physiolog` value when `d.isXAPhysio` is true and `_acq-pmu` when `d.isCMRRPhysio` is true. The splice finds an existing `_acq-<value>` (reproin guarantees the value contains no underscores because `reproinSanitize` strips them) and overwrites the value; if no `_acq-` exists, it inserts `_acq-<tag>` before `_run-` (or appends if no `_run-` either). Net effect on the mixed BOLD+SBRef physio dataset: dcm2niix now produces eight files (4 PhysioLog + 4 PMU) instead of overwriting four. The Python rescue still strips the modality infix and moves to the main tree, ending with stems like `sub-X_ses-Y_task-Z_acq-physiolog_..._recording-cardiac_physio.tsv.gz` and `sub-X_ses-Y_task-Z_acq-pmu_..._recording-cardiac_physio.tsv.gz`.

BIDS-pairing trade-off: per the BIDS physiological-recordings spec the physio file's entities MUST match the parent NIfTI's. With this disambiguation the physio's `_acq-` differs from the parent BOLD's, so entity-based pairing tools (fmriprep etc.) will NOT auto-pair them — users must manually associate physio→imaging by hand. The alternative (folding the disambiguator into the `_recording-LABEL` token, e.g. `_recording-cardiacpmu`) would preserve pairing but lose the explicit `_acq-` tag the maintainer chose for human-readability. Documented; revisit if downstream tooling complaints surface.
