...
- **Siemens PhysioLogging / CMRR PMU TSV** (`_recording-<label>_physio.tsv.gz` + `.json`): per-stream layout is **`[<signal>, trigger, <label>_trigger]`** when firmware peaks are present, otherwise `[<signal>, trigger]`. Column 2 (`trigger`) is **scanner-volume-only** (from `ACQUISITION_INFO`, rasterised by `physioBidsRasterTrigger` at `nii_dicom_batch.cpp:~9836`) — byte-compatible with `bidsphysio` and consistent with the BIDS spec's "continuous measurement of the scanner trigger signal" wording. Column 3 (`<label>_trigger`, e.g. `cardiac_trigger`, `respiratory_trigger`) is the deliberate deviation: it carries firmware-detected R-wave / respiratory events captured per-stream by `cmrrPhysioParseLine` at `nii_dicom_batch.cpp:~10413`. CMRR PULS/RESP/EXT/ECG rows with a 4th `*_TRIGGER` SIGNAL token use the reserved sentinel `VALUE=2048` at off-grid odd ticks; following the Siemens reference `physiodcm2tsv.py` policy we **drop the sentinel from the sample stream** (bidsphysio's `dcm2bidsphysio.py:213` "we can ignore" comment leaks `2048` into the waveform — do not regress to that). The captured trigger tics live on per-stream `triggerTics` arrays and are rasterised onto column 3 via the same nearest-grid snap. `physioBidsFillUniform` (`~9858`) now takes `peakTics`/`peakN` and yields a parallel `outPeakTrigger` raster; `xaPhysioWriteStreamFiles` (`~10002`) writes the 3-column TSV when `peakLabel`/`peakTrigger` are non-NULL. The XA-line gzip-XML path passes `NULL` for the peak channel (its `<PhysioTriggers>` XML element isn't parsed yet) and stays 2-column. Validation corpus: [dcm_qa_physio](https://github.com/neurolabusc/dcm_qa_physio) (`In/7` cardiac = 4 column-2 / 27 column-3; respiratory = 4 / 9).
