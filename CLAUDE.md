...
- **Physio writer error propagation** (audit 2026-06-11 M1+M2): `xaPhysioWriteStreamFiles` (`nii_dicom_batch.cpp:~10042`) returns `bool` — false on any cJSON alloc / fopen / fwrite-short / deflate / Z_FINISH-not-reached failure, with partial files `unlink()`'d so a retry doesn't observe half-written sidecars. `physioBidsEmitStream` returns a tri-state `PhysioEmitStatus` (`kPhysioEmitOk / kPhysioEmitSkipped / kPhysioEmitFailed`); `peakN > 0 && uPeak == NULL` (raster OOM) maps to `Failed`, distinguishing "rasterizer OOM dropped a column" from "no sensors connected" (the latter is `Skipped`, returns `EXIT_SUCCESS`; the former increments `writeFails`, returns `EXIT_FAILURE`). Both `xaPhysioConvert` and `cmrrPhysioConvert` tally `writeFails` and `printError` + `EXIT_FAILURE` when any stream tried-and-failed. Decompressed TSV output is byte-identical with prior dcm_qa_physio Ref/ (gzip noise in compressed bytes is pre-existing and not from these changes).
