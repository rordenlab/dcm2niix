...
- **C-side scope going forward**: raw FID layout, affine construction, suffix classification (`_svs` / `_mrsref` / `_mrsi`), BIDS-MRS required + recommended sidecar fields (including `VOI` 4x4 matrix for MRSI), Philips classic 2× `_mrsref` companion, single-source-of-truth helpers (`mrsSpectralWidthHz`, `mrsIsStandaloneWaterRef`, `initTDTI4D`). MRSI writer (`saveDcm2NiiMRSI`, Phase 6 close-out) handles Enhanced CSI (Siemens + UIH same SOP `1.2.840.10008.5.1.4.1.1.4.2`), classic Siemens VB/VE CSI (SOP `1.3.12.2.1107.5.9.1` with CSA Rows/Columns/NumberOfFrames + PixelSpacing extraction), and the VB/VE 3D CSI rotated-slice case (slice-axis half-shift `sz = 0.5*(m20+m21)`). Vendor-state-specific reshapes/splits explicitly out — those live in `tools/mrs_post.py`.
