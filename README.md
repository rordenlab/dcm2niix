...
DICOM provides many ways to store/compress image data, known as [transfer syntaxes](https://www.nitrc.org/plugins/mwiki/index.php/dcm2nii:MainPage#DICOM_Transfer_Syntaxes_and_Compressed_Images). The [COMPILE.md file describes details](./COMPILE.md) on how to enable different options to provide support for more formats.
 ...
 - Single-voxel MR spectroscopy (Siemens VB/VE/XA SVS, Philips classic SVS, UIH SVS) is converted to BIDS-MRS compliant complex-float NIfTI + `.json` sidecars. Philips classic 2× payloads emit a paired `<stem>_mrsref.nii.gz` water-reference companion; standalone water-reference acquisitions (`wrsoff` / `no_Water_Suppression` series-naming) are relabeled `_mrsref` with `WaterSuppressed: false`. CSI/MRSI is currently rejected at the writer (lands in `Unknown/` rather than being mislabeled). Ported from [spec2nii](https://github.com/wtclarke/spec2nii) (BSD-3-Clause, William Clarke, U. Oxford).
