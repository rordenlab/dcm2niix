...
- Rename mode (`-r y`) shares that contract: `copyFile` treats an unreadable source or a failed write/close as fatal and aborts with `kEXIT_RENAME_ERROR`, never counting the file as renamed. An existing target is byte-compared: identical inputs are skipped and summarized in one line, distinct inputs collide and return `kEXIT_SOME_OK_SOME_BAD`. Both were fail-open regressions this cycle; do not relax them.
