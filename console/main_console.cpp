...
void showHelp(const char * argv[], struct TDCMopts opts) {
    ...
    printf("  -f : filename (%%c=comments %%d=description %%f=folder name %%i ID of patient %%m=manufacturer %%n=name of patient %%p=protocol, %%q=sequence %%s=series, %%t=time, %%z sequence name; default '%s')\n",opts.filename);
    printf("  -h : show help\n");
