...
{ // "/usr/path/filename.exe" -> "filename.exe"
...
void showHelp(const char *argv[], struct TDCMopts opts)
{
    ...
    printf("  -a : adjacent DICOMs (images from same series always in same folder) for faster conversion (n/y, default "
           "n)\n");
    ...
    printf("  -c : comment stored in NIfTI aux_file (up to 24 characters e.g. '-c VIP', empty to anonymize e.g. "
           "0020,4000 e.g. '-c \"\"')\n");
    printf("  -d : directory search depth. Convert DICOMs in sub-folders of in_folder? (0..9, default %d)\n",
           opts.dirSearchDepth);
#ifdef myEnableJNIFTI
    printf("  -e : export as NRRD (y) or MGH (o) or JSON/JNIfTI (j) or BJNIfTI (b) instead of NIfTI (y/n/o/j/b, "
           "default n)\n");
#else
...
#define kQstr ""
#endif
    printf("  -f : filename (%%a=antenna (coil) name, %%b=basename, %%c=comments, %%d=description, %%e=echo number, "
           "%%f=folder name, %%g=accession number, %%i=ID of patient, %%j=seriesInstanceUID, %%k=studyInstanceUID, "
           "%%m=manufacturer, %%n=name of patient, %%o=mediaObjectInstanceUID, %%p=protocol,%s %%r=instance number, "
           "%%s=series number, %%t=time, %%u=acquisition number, %%v=vendor, %%x=study ID; %%z=sequence name; default "
           "'%s')\n",
           kQstr, opts.filename);
    printf("  -g : generate defaults file (y/n/o/i [o=only: reset and write defaults; i=ignore: reset defaults], "
           "default n)\n");
    printf("  -h : show help\n");
    printf("  -i : ignore derived, localizer and 2D images (y/n, default n)\n");
    printf("  -J : JSON metadata (y/n, default n)\n");
    char max16Ch = 'n';
    if (opts.isMaximize16BitRange == kMaximize16BitRange_True)
        max16Ch = 'y';
    if (opts.isMaximize16BitRange == kMaximize16BitRange_Raw)
        max16Ch = 'o';
    printf("  -l : losslessly scale 16-bit integers to use dynamic range (y/n/o [yes=scale, no=no, but uint16->int16, "
           "o=original], default %c)\n",
           max16Ch);
    printf("  -m : merge 2D slices from same series regardless of echo, exposure, etc. (n/y or 0/1/2, default 2) [no, "
           "yes, auto]\n");
    ...
    printf("  -q : only search directory for DICOMs (y/l/n, default y) [y=show number of DICOMs found, l=additionally "
           "list DICOMs found, n=no]\n");
...
// n.b. the optimal use of pigz requires pipes that are not provided for Windows
    ...
    if (strlen(opts.pigzname) > 0)
        ...
        printf("  -z : gz compress images (y/o/n/3, default %c)  [y=pigz(MISSING!), o=optimal(requires pigz), n=no, "
               "3=no,3D]\n",
               gzCh);
#else
#ifdef myDisableMiniZ
    printf(
        "  -z : gz compress images (y/o/i/n/3, default %c) [y=pigz, o=optimal pigz, i=internal:zlib, n=no, 3=no,3D]\n",
        gzCh);
#else
    printf(
        "  -z : gz compress images (y/o/i/n/3, default %c) [y=pigz, o=optimal pigz, i=internal:miniz, n=no, 3=no,3D]\n",
        gzCh);
#endif
#endif
    printf("  --big-endian : byte order (y/n/o, default o) [y=big-end, n=little-end, o=optimal/native]\n");
...
#define kOS "Windows"
    ...
    while (i < (argc))
    { //-1 as final parameter is DICOM directory
        ...
        { // command
            ...
            else if ((argv[i][1] == 'b') && ((i + 1) < argc))
            {
                ...
                { //"-bi M2022" provide BIDS subject ID
                    i++;
                    snprintf(opts.bidsSubject, kOptsStr - 1, "%s", argv[i]);
                }
                else if (argv[i][2] == 'v')
                { //"-bv 1222" provide BIDS subject visit
                    i++;
            ...
            else if ((argv[i][1] == 'g') && ((i + 1) < argc))
            {
                ...
                if (((argv[i][0] == 'i') || (argv[i][0] == 'I')) && (!isResetDefaults))
                {
                    ...
                    i = 0; // re-read all settings for this pass, e.g. "dcm2niix -f %p_%s -d o" should save filename as
                           // "%p_%s"
                }
                if (((argv[i][0] == 'o') || (argv[i][0] == 'O')) && (!isResetDefaults))
                {
                    ...
                    setDefaultOpts(&opts, argv);
                    // this next line is optional, otherwise "dcm2niix -f %p_%s -d o" and "dcm2niix -d o -f %p_%s" will
            ...
            else if (argv[i][1] == 'J')
            {
                ...
                // For more information extraction, disable BIDS anonymization when using -J option
                opts.isAnonymizeBIDS = false;
...
#endif
    if ((opts.isRenameNotConvert) && (!isOutNameSpecified))
    { // sensible naming scheme for renaming option
// strcpy(opts.filename,argv[i]);
// 2019 - now include "%o" to append media SOP UID, as instance number is not required to be unique
    ...
    if (argc == (lastCommandArg + 1))
    { //+1 as array indexed from 0
    ...
    {
        printf("Warning: only processing last of %d input files (recompile with 'myEnableMultipleInputs' to "
