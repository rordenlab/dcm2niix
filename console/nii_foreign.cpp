...
unsigned char * nii_readEcat7x(const char * fname, struct nifti_1_header *nhdr) {
//data type
#define	ECAT7_BYTE 1
...
#define ECAT7_3DSCANFIT 14
    ...
    if (num_vol > 1) //to do: multi-volume files
        printMessage("Only reading first volume from ECAT file with %d frames and %d bed positions\n", mhdr.num_frames, mhdr.num_bed_pos);
    */
    //read list matrix
    ecat_list_hdr lhdr;
	...
	nhdr->bitpix = bytesPerVoxel * 8; //8 bits per byte
	...
	//load image data
	num_vol = 1; //666
	nhdr->dim[4]=num_vol; //volumes
	if (num_vol > 1) nhdr->dim[0]=4; //volumes
	...
	printMessage("ECAT support VERY experimental (Spatial transforms unknown)\n");
    free (imgOffsets);
    fclose(f);
...
unsigned char * nii_readEcat7(const char *fname, struct TDICOMdata *dcm, struct nifti_1_header *hdr) {
//data type
#define	ECAT7_BYTE 1
#define	ECAT7_VAXI2 2
#define ECAT7_VAXI4 3
...
#define ECAT7_3DSCANFIT 14
    ...
    */
	...
	dcm->xyzMM[4] = 123; //TR: TO DO
	dcm->xyzDim[1] = ihdr.x_dimension;
	dcm->xyzDim[2] = ihdr.y_dimension;
	dcm->xyzDim[3] = ihdr.z_dimension;
    dcm->xyzDim[4] = num_vol;
    strcpy(dcm->patientName, mhdr.patient_name);//patient_id[16], patient_name[32]
    strcpy(dcm->patientID, mhdr.patient_name);
    strcpy(dcm->seriesDescription, mhdr.study_description);
    strcpy(dcm->protocolName, mhdr.study_type);
    strcpy(dcm->imageComments, mhdr.isotope_name);
...
int nii_createFilename(char * niiFilename, struct TDCMopts opts) {
    char pathoutname[1024] = {"/Users/rorden/Documents/Cocoa/dcm2niix/console/mega"};
...
int  open_foreign (const char *fn, struct TDCMopts opts){
	printMessage("ECAT images not yet supported\n"); return EXIT_FAILURE;
	struct nifti_1_header hdr;
	struct TDICOMdata dcm = clear_dicom_data();
	//bool swapEndian;
	unsigned char * img = NULL;
	char niiFilename[1024] = {"/usr/myFolder/dicom.dcm"};
	int ret = nii_createFilename(niiFilename, opts);
	if (ret != EXIT_SUCCESS) return ret;
	//img = nii_readEcat7(fn, &hdr);
	img = nii_readEcat7(fn, &dcm, &hdr);
