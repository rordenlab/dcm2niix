...
| NumberShots                        |      | Excitations per inversion block. Siemens MP2RAGE only: CSA `sFastImaging.lTurboFactor`, gated on `lInvContrasts == 2`. Reported as `[before, after]` (excitations before/after the k-space centre) when the reordering is unambiguous — CSA `sKSpace.Reordering3D`/`unReordering` linear or centric with partial Fourier off — else as the scalar turbo factor | B          |
...
| RepetitionTimePreparation          | s    | Preparation-block repeat time. Siemens MP2RAGE: DICOM tag 0018,0080, which for MP2RAGE is the inversion-to-inversion time rather than an excitation TR. (Siemens ASL sequences also populate this field, in milliseconds — a pre-existing inconsistency) | B          |
...
Siemens also includes some sequence information in the private MRPhoenixProtocol (0021,1019) tag. You can view this with [gdcmdump](https://gdcm.sourceforge.net/html/gdcmdump.html), e.g. `gdcmdump --mrprotocol img.dcm`. Fields that dcm2niix inspects include `sPat.lAccelFact3D `, `sPat.lAccelFactPE`, `sPat.lRefLinesPE`, `sPat.ucPATMode`, `sSliceArray.lConc` (3D-EPI multi-echo shots, see `MultiEchoShots`), and — for MP2RAGE — `sFastImaging.lTurboFactor` with `sKSpace.Reordering3D`/`sKSpace.unReordering` (see `NumberShots`). The behavior of dcm2niix will be more well documented as our understanding of this tag improves.

**MP2RAGE and `RepetitionTimeExcitation` (TR_GRE).** Inverting an MP2RAGE `UNI` image to quantitative T1 needs the inner GRE repetition time, and dcm2niix does not emit it because Siemens does not appear to export it. For the product `tfl` sequence on syngo MR XA60 it is absent from every export we have checked: `sFastImaging.lEchoSpacing` is not written to the Phoenix protocol at all (not for the EPI series of the same session either, so `VendorReportedEchoSpacing` is likewise missing there), `alTD[]` and `sWipMemBlock.alFree[]`/`adFree[]` are declared but empty, the `PhoenixZIPReport` SR duplicates the same ASCCONV text rather than adding to it, and every frame of the enhanced object reports the same `FrameAcquisitionDateTime` (0018,9074) and a whole-series `FrameAcquisitionDuration` (0018,9220), so per-frame timing does not encode it either. Do **not** substitute `sFastImaging.lEchoTrainDuration`: it reports a constant `700` for every sequence of a session, including single-echo GRE ([issue 127](https://github.com/rordenlab/dcm2niix/issues/127)). Users needing TR_GRE must currently read it from the scanner protocol printout or the raw (twix) header.
