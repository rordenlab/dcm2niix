...
It is very important that your file naming disambiguates all your images. For example, consider a naming scheme that only used the image number (`-f %r.dcm`) and was applied to multiple series (each which had an image number 1,2,...). When two inputs map onto the same output name, dcm2niix compares their bytes. An exact duplicate is skipped, and the count is reported once at the end (`32 duplicate(s) skipped (byte-identical to already-saved file)`). Distinct images that collide are each reported (`Naming conflict: distinct input '...' maps to existing '...' — use a more specific -f template`), the remaining files are still copied, and the run ends with exit code 8 so a script can detect it.
